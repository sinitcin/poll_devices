...
fn thread_start(addr: String, sender: Sender<bool>) {
    ...
    for stream in listener.incoming() {
        ...
        thread::spawn(move || {
            ...
            let response =
                libengine::processing(&(&*String::from_utf8_lossy(&request))[0..nbytes]).unwrap();
...
pub fn debug_test() {
    let server: DebugServer = DebugServer {
    ...
    let mut stream = TcpStream::connect("127.0.0.1:8080")
        .expect("Не могу подключиться к серверу...");
    let command = json!({
        "action": "init"
    });
    ...
    assert_eq!(
        buffer,
        "{\"action\":\"init\",\"code\":200,\"guid\":\"Тестовый GUID\"}".to_string()
    );
}
